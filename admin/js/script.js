...
$(document).ready(function() {
	...
	if ($('#summernote').length > 0) {
		...
		$('#form-create-page').submit(function(ev){
	...
	var listSortableFunc = function() {
		if ($('.listsortable').length > 0) {
			...
			var group = $(".listsortable").sortable({
				handle: '.menuItemDrag',
				group: 'theMenuList',
				nested: true,
	...
	$('body').on('hidden.bs.modal', '.modal', function () {
		if ($(this).hasClass('note-image-dialog') || $(this).hasClass('note-link-dialog') || $(this).hasClass('note-video-dialog') || $(this).hasClass('note-help-dialog')) {
			console.log("no touchy note dialogs...")
		} else {
			$(this).removeData('bs.modal');
			$(this).empty();
			console.log('clearing modal content...');
			$('#modal').html('<div id="modal-preloader"></div>');
		}
	});
    ...
    var photoManagerUpdate = function() {
    	...
    	if ($('#photo-manager-box').length>0) {
			...
			/*$('.photo-filter a').on('click', function(ev){
				ev.preventDefault();
