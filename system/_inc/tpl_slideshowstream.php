            ...
            <div class="container-fluid">
                <div id="carousel">
                    ...
                    <% _.each(slideshowstream, function(slide) { %>
                    ...
                    <div id="carousel-filmstrip">
                        <ul>
                            <% _.each(slideshowstream, function(th_slide) { %>
                            <% _.each(th_slide.get('thumbnails'), function(th_src,th_i) { %>
                            <% if (th_slide.get('slide_id') == sid) { %>
                                <li data-id="<%=th_i+1%>"><a href="#/slideshow/<%=sid%>/<%=th_i+1%>"><img src="<%= th_src %>"></a></li>
                            <% } %>
                            <% }) %>
                            <% }) %>
                        </ul>
                    ...
                    <ul id="carousel-slides">
                        <li data-id="0" class="prevSlide"><img src="system/images/blank.gif" width="1" height="1"></li>
                            ...
                            <% var i = 0; _.each(slideshowstream, function(slide) { %>
            ...
            <script>
                ...
                var image_list = new Array(
                    <% _.each(slideshowstream['0'].get('photos'), function(prevphoto,i) { %>
                    '<%= prevphoto %>',
                    <% }) %>
                    <% _.each(slideshowstream['0'].get('thumbnails'), function(thphoto,i) { %>
                    '<%= thphoto %>',
                    <% }) %>
                    ...
                    */
                    function Carousel(element)
                    {
                        ...
                        this.showPane = function(index, animate) {
                            ...
                            var t_slide_total = 0;
                            var t_slide_num = 0;
                            for (var i = 0; i < slideshowstreamDump.length; i++) {

                                if (slideshowstreamDump[i]['slide_id'] == sid) {
                                    t_slide_id = slideshowstreamDump[i]['slide_id'];
                                    t_slide_num = i;
                                    t_slide_total = slideshowstreamDump[i]['photos'].length;
                                }
                            }
                            ...
                            if (current_pane == t_slide_total+1) {
                                ...
                                current_pane = slideshowstreamDump['0']['photo_total'];
                                $('#carousel').stop().fadeOut('500',function(){
                                    self.showPane(current_pane);
                                    $('#carousel').fadeIn('500');
                                });
                                ...
                                var shareImg = slideshowstreamDump[t_slide_num]['photos'][current_pane-1];
